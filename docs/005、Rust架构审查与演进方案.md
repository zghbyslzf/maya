...
- 代码中不再以 `ok()`/`flatten()` 静默丢弃遍历错误；
- 权限错误测试能得到具体路径和底层错误；
- CLI 能区分“全部成功”和“带警告完成”。

### 4.2 修复嵌套 `node_modules` 删除错误
...
- 嵌套 `node_modules` 只生成一个有效删除目标；
- 删除结果报告实际删除数量；
- 同时存在多个同级 `node_modules` 时全部删除；
- 补充嵌套目录和遍历期间目标消失的回归测试。

### 4.3 所有破坏性写入改为临时产物后替换
...
- 模拟编码、磁盘写入或 FFmpeg 中途失败时，原产物保持不变；
- 成功后不存在遗留的 `.maya-tmp-*`；
- ZIP、图片和视频共用同一套经过测试的提交/清理策略。

### 4.4 以进程退出状态和产物契约判断 FFmpeg 成功
...
- 假 FFmpeg 返回非零退出码但创建了 playlist 时，命令仍判定失败；
- 退出码为 0 但产物不完整时，命令判定失败；
- 错误包含命令、工作目录、退出码和有界的 stderr；
- 旧的成功产物不会被一次失败转换删除。

## 5. P1：让边界、输入、输出和失败可被理解
...
### 5.3 用 Options 和 Report 替代长参数、布尔值和裸元组
...
### 阶段 1：明确输入输出边界（预计 2～4 天）

- CLI 参数改为类型化枚举和 options；
- 所有业务入口显式接收路径；
- 用 typed report 替代裸元组；
- 输出和进度条迁移到 CLI presenter；
- 结构化错误、失败策略和退出码；
- 增加 `--quiet`、`--no-progress`，JSON 可随后增量加入。
...
- [ ] 所有公共业务 API 都显式接收输入路径，不读取全局当前目录；
- [ ] 业务 crate 不直接 `println!`/`eprintln!`，终端展示集中在 CLI；
- [ ] CLI 参数使用类型表达，非法组合尽量在解析阶段拒绝；
- [ ] 批处理返回 typed report，能区分成功、失败、跳过和警告；
- [ ] 遍历错误不会被 `ok()` 或 `flatten()` 静默丢弃；
- [ ] 所有覆盖式写入使用临时产物验证后替换；
- [ ] FFmpeg 以退出状态和完整产物契约判断成功；
- [ ] 嵌套 `node_modules` 回归测试通过；
- [ ] Git 逻辑不依赖英文 stderr；
- [ ] 部分失败有稳定的报告和非零退出语义；
